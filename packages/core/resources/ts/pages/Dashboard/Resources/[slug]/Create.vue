...
const storeRoute = useRoute("dashboard.resources.slug.store", {
  slug,
});
const storeHandler = () => {
...
<template>
  <DashboardPage
    :title="
      $t('page.dashboard.resources.create.meta.title', {
        modelTitle: title,
      })
    "
  >
    <DashboardPageHeader>
      <template #title>
        {{
          $t("page.dashboard.resources.slug.create.header.title", {
            modelTitle: title,
          })
        }}
      </template>
      <template #description>
        {{ $t("page.dashboard.resources.slug.create.header.description") }}
      </template>
    </DashboardPageHeader>
    ...
    <UiCard>
      <UiCardHeader>
        <UiCardTitle>
          {{ $t("page.dashboard.resources.slug.create.general.title") }}
        </UiCardTitle>
        <UiCardDescription>
          {{ $t("page.dashboard.resources.slug.create.general.description") }}
        </UiCardDescription>
      </UiCardHeader>
      <UiCardContent>
        <UiAutoForm
    ...
    <div class="flex justify-end gap-2">
      <UiButton @click="storeHandler()" :disabled="storeButtonIsDisabled">
        {{ $t("page.dashboard.resources.slug.create.action.store.label") }}
      </UiButton>
    </div>
