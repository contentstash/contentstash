...
export const createContentStashApp = (params: any) => {
    ...
    return {
        ...
        resolve: (name: string) => {
            ...
            const appPages2 = import.meta.glob('./../../../resources/js/Pages/**/*.vue',  { eager: true })
            const corePages2 = import.meta.glob('./../../../vendor/contentstash/core/resources/js/Pages/**/*.vue',  { eager: true })
            ...
            const addPages = (sourcePages: any, basePath: any) => {
                ...
                return Object.entries(sourcePages).forEach(([path, component]: [
                    string,
                    any
                ]) => {
                    pages[path.replace(basePath, '')?.replace('.vue', '')] = component;
